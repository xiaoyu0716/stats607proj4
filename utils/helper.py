...
def has_svd(forward_op):
    ...
    # Check for methods (V, Ut, M, S) used by DDNM
    if (hasattr(forward_op, 'V') and callable(getattr(forward_op, 'V')) and
        hasattr(forward_op, 'Ut') and callable(getattr(forward_op, 'Ut')) and
        hasattr(forward_op, 'M') and
        hasattr(forward_op, 'S')):
        return True
    # Also check for attributes (U, S, Vt) for backward compatibility
    elif hasattr(forward_op, 'U') and hasattr(forward_op, 'S') and hasattr(forward_op, 'Vt'):
        return True
    else:
        return False
