# MCG-diff Nullspace Variance Complete Fix Patch

## Summary

This patch implements proper separation of conditional (observed) and prior (nullspace) reverse diffusion chains, ensuring nullspace variance matches theoretical expectations (ratio ≈ 5-10 instead of ≈ 1).

## Changes

### 1. Added `_reverse_step_prior()` method

Implements pure prior reverse diffusion for nullspace dimensions:
- Uses same noise schedule as conditional chain
- NO data guidance (no likelihood, no score)
- Formula: `x_prior = scaling_factor * x_t + sqrt(factor) * noise`

### 2. Added `_reverse_step_conditional()` method

Implements data-guided reverse diffusion for observed dimensions:
- Uses measurement y for guidance
- Formula: `x_post = K * observation + (1-K) * x_next_t + sqrt(K) * sigma_next * noise`

### 3. Modified main inference loop

- Samples shared noise ONCE per step (both chains use same noise)
- Separates conditional and prior updates
- Combines using mask: `x_t = mask * x_post + (1-mask) * x_prior`
- Removed all old nullspace hack code
...
@@ -209,20 +251,30 @@ class MCG_diff(Algo):
...
1. ✅ **Nullspace uses pure prior diffusion**: No score contamination
2. ✅ **Shared noise**: Both chains use same random noise per step
3. ✅ **Clean separation**: Observed and null dims handled separately
4. ✅ **Removed hacks**: All old nullspace correction code removed
...
Expected output:
- Null variance ≈ 1.0-1.5 (vs 0.13 before)
- Ratio ≈ 5-10 (vs 0.8 before)
