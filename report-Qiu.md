...
First, the framework follows a \emph{modular programming} philosophy: the
...
Second, I applied the principle of \emph{reducing problem dimensionality for
...
Third, the project relies heavily on \emph{simulation-based validation and
...
Overall, I use the modular design, efficient computation, and
...
3. Passing tests is not the same as being correct.
One of the main challenges was that the code often executed successfully but produced visually impossible reconstructions. This revealed a key lesson: in uncertainty quantification, correctness cannot be judged by reconstruction quality or stability alone. We learned to rely on systematic testing—such as identity forward models, SVD-based variance decomposition, and coverage diagnostics—to detect subtle but critical implementation errors (e.g., incorrect basis transforms or null-space updates).

1. Debugging scientific code requires hypothesis-driven testing.
When results did not match expectations, the solution was rarely a single bug fix. Instead, we designed a sequence of increasingly simplified tests to localize the source of error. This process—reducing the problem to analytically tractable cases and checking each assumption—was essential for resolving issues such as variance collapse or explosion. The experience reinforced that debugging research code is fundamentally an experimental and scientific process, not just a software engineering task.
